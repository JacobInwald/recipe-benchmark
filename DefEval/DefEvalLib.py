...
def _process(p, env=None):
    os.environ['GLIBC_TUNABLES'] = 'glibc.cpu.hwcaps=SHSTK,IBT:glibc.cpu.x86_ibt=on'
    
    emulate = os.environ.get('SDE_PATH', default='')
    _p = p
    if emulate != '':
        p = [emulate+'/sde64', '-cet --', p]
        _p = ' '.join(p) 
    if env:
        return pwn.process(_p, env=env, shell=True)
    else:
        return pwn.process(_p, shell=True)
...
def error(s):
	...
	print(CRED + "Error: {}!".format(s) + CEND)
...
class recipe_stack_BoundOFlow(recipe_attack):
	def start(self):
		p = _process(self.elf, env=self.env)
		if self.valgrind==1:
